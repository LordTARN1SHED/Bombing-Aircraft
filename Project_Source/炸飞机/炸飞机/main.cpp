...
int main() {
	...
	while (c != '1' && c != '2') {
		...
		printf("游戏说明：\n飞机机头用数字2表示，机身用数字1表示，一架飞机由一个机头和九个机身组成（即一个2和九个1）\n飞机位置坐标格式为:x(空格)y(空格)z，其中x和y分别是横纵坐标（坐标范围是0-9），z代表方向：u是上，d是下，l是左，r是右\n\n");
		initmap3();
		printf("上图中实例分别是四架飞机，他们代表了四种飞机方向,。\n他们的头部坐标从上到下从左到右依次是：2 0 u ，7 3 d ， 0 7 l ， 9 7 r 。\n例如：2 0 u 代表飞机机头位置在坐标（2，0）飞机方向朝上 \n");
